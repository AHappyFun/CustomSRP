...
LumaNeighborhood GetLumaNeighborhood(float2 uv)
{
    ...
    luma.highest = Max3(Max3(luma.m, luma.n, luma.e), luma.s, luma.w);
    luma.lowest = Min3(Min3(luma.m, luma.n, luma.e), luma.s, luma.w);
...
float4 FXAAPassFragment(Varyings input) : SV_TARGET
{
    ...
    float blendFactor = max(GetEdgeBlendFactor(luma, edge, input.screenUV), GetSubpixelBlendFactor(luma));
    //return blendFactor;
