...
public class LightProbesTetrahedralGrid : MonoBehaviour
{
            ...
            if (l % 2 == 0)
            {
                for (uint i = 0; i < iterations; i++)
                {
                    ...
                    for (uint s = 1; s < subdiv; s++)
                    {
                        ...
                        Vector3 rightDownLeftDownSubdiv =
                            rightDownCorner + rightDownLeftDown * (float) s / (float) subdiv;
    ...
    void GenerateRing(TriangleProps props, float radius, float innerRadius, float height, uint levels,
        List<Vector3> outPositions)
    {
        ...
        for (uint level = 0; level < levels; level++)
        {
            ...
            for (uint layer = 0; layer < layers; layer++)
            {
                ...
                float tIterations = (tLayer * (radius - innerRadius) + innerRadius - kMinInnerRadius) /
                ...
                for (uint slice = 0; slice < slices; slice++)
                {
                    Quaternion rotation = Quaternion.Euler(0.0f,
                        (slice + iterationOffset0 + layerSliceOffset) * 360.0f / (float) slices, 0.0f);
...
public class LightProbesTetrahedralGridInspector : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        {
            LightProbesTetrahedralGrid t = target as LightProbesTetrahedralGrid;
            t.Generate();
        }
    }
}
